...
df_task1_vcoef <- read.csv('../results/mid_van_2019_perception/ddm/m2_slps_posterior.csv')
...
figure <- ggplot(df_task1_vcoef, aes(x = df_task1_vcoef[, 1])) +
  geom_density() +
...
extract_vcoef <- function(suffix, col_index = 1) {
  path <- glue::glue("../results/mid_van_2019_perception/ddm/m2_{suffix}_posterior.csv")
  if (!file.exists(path)) {
...
pipeline_labels <- data.frame(
  ...
  label = c('Binwise Build-up rate',
            'Trialwise Build-up rate',
...
# 7. plot
figure_main <- ggplot(df_long, aes(x = v)) +
  geom_density() +
  geom_segment(
    data = stats_df,
...
# Save main effect plot
ggplot2::ggsave(
  filename = '../figure/perc/main_effect.jpg',
  plot = figure_main,                
  width = 200,                       
  height = 200*2/3,                       
  units = "mm",
...
# Use for loop to plot interaction plot

plot_list <- list()
for (col_interaction in 2:4){
  # 3. Use function to all models
  df_list <- map_dfr(suffixes, ~ extract_vcoef(.x, col_index = col_interaction))
  ...
  figure <- ggplot(df_long, aes(x = v)) +
    ...
    labs(y = 'Posterior probability', x = 'Coefficient of CPP effect on drift rate') +
    theme_bw() +
    coord_cartesian(xlim = c(-1.5, 1), clip = "off") +
    theme(
      axis.text = element_text(size = 8),
      panel.grid = element_blank(),
      text = element_text(family = 'serif'),
...
for (i in seq_along(plot_list)) {
  ...
  filename <- glue::glue("../figure/perc/interaction_effect_{col_idx}.png")
  ggsave(
    filename = filename,
    plot = plot_list[[i]],
    width = 200,                       
